...
const applyStyles_1 = require("./applyStyles");
describe('applyStyles', () => {
    ...
    test('should return template content and 1 change if content is different', () => {
        ...
        const result = (0, applyStyles_1.applyStyles)('<w:styles>Old styles</w:styles>', { templateContent: mockTemplateContent });
        expect(result.xml).toBe(mockTemplateContent);
        expect(result.changes).toBe(1);
    });
    test('should return original content and 0 changes if content is the same', () => {
        // Теперь передаем templateContent напрямую
        const result = (0, applyStyles_1.applyStyles)(mockTemplateContent, { templateContent: mockTemplateContent });
        expect(result.xml).toBe(mockTemplateContent);
