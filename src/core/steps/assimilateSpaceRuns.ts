...
export function assimilateSpaceRuns(xml: string, params: any): StepResult {
  ...
  const rootTag = 'customroot'; // Наша собственная обертка
  const namespace = 'http://schemas.openxmlformats.org/wordprocessingml/2006/main';
  const wrappedXml = `<${rootTag} xmlns:w="${namespace}">${xml}</${rootTag}>`; // Обертка с пространством имен
  ...
  if (changes > 0) {
    ...
    // Извлекаем контент из нашей обертки
    ...
    // Если что-то пошло не так, возвращаем исходный XML и 0 изменений
